...
class forcefield_optimization_strategy(optimization.optimization_strategy):
    ...
    bounds
        Mapping from model numbers (eg, 0 for bonds, 1 for angles, etc) to
        splitter/extender config
    direct_enable = False
        Turn on analytic searching for SMARTS patterns if numerical search finds
        too few splits
    direct_limit = 10
    ...
    enable_merge
        Enable the MERGE operation during candidate parameter generation.
    enable_split
        Enable the SPLIT operation during candidate parameter generation.
    enable_modify
        Enable the MODIFY operation during candidate parameter generation.

    modify_outofplane_frequency_limit
        Set the largest out-of-plane frequency (also known as improper
        torsion periodicity) that can be generated by the MODIFY operation
        during parameter generation.
    modify_torsion_frequency_limit
        Set the largest torsion frequency (also known as proper torsion
        periodicity) that can be generated by the MODIFY operation during
        parameter generation.

    tree_iterator
        Change this to visit clusters of SMARTS in a particular order, filter
        nodes, etc.

    macro_accept_max_total
        How many candidates to pass from one macro step to the next. ``1`` will
        be fastest, but produce less optimal force fields. ``0`` means
        unlimited.

    micro_accept_max_total
        How many candidates to pass from one micro step to the next. ``1`` will
        be fastest, but produce less optimal force fields. ``0`` means
        unlimited. Less relevance in force field fitting than other
        ``accept_max`` options.

    macro_accept_max_per_cluster
        How many candidates of a given parameter to pass from one macro step to
        the next. Bounded above by ``macro_accept_max_total``. ``1`` will be
        fastest, but produce less optimal force fields. ``0`` means limited only
        by ``macro_accept_max_total``.

    micro_accept_max_per_cluster
        How many candidates of a given parameter to pass from one micro step to
        the next. Bounded above by ``micro_accept_max_total``. ``1`` will be
        fastest, but produce less optimal force fields. ``0`` means limited only
        by ``micro_accept_max_total``.

    prune_empty
        If we accept too many operations, some can match nothing due to
        unexpected occlusion. With this enabled, merging is shortcutted to
        prevent zero-matching SMARTS from being added.

    enable_reset_bond_lengths
        If ``True``, initialize bond lengths from the reference data. If
        ``False``, initialize bond lengths to the parameter value from the
        parent force field.
    enable_reset_angle_lengths
        If ``True``, initialize bond angles from the reference data. If
        ``False``, initialize bond angles to the parameter value from the
        parent force field.

    enable_reset_bond_stiffness
        If ``True``, initialize bond stiffness from the reference data
        hessians. If ``False``, initialize bond stiffness to the parameter value
        from the parent force field. Requires hessians in reference data.
    enable_reset_angle_stiffness
        If ``True``, initialize angle stiffness from the reference data
        hessians. If ``False``, initialize angle stiffness to the parameter
        value from the parent force field. Requires hessians in reference data.
    enable_reset_torsion_stiffness
        If ``True``, initialize torsion stiffness from the reference data
        hessians. If ``False``, initialize torsion stiffness to the parameter
        value from the parent force field. Requires hessians in reference data.
    enable_reset_outofplane_stiffness
        If ``True``, initialize out-of-plane stiffness from the reference data
        hessians. If ``False``, initialize out-of-plane stiffness to the
        parameter value from the parent force field. Requires hessians in
        reference data.
    hessian_projection_method
        The hessian projection method from parameter resets.

    enable_dihedral_periodicity_reset
        If ``True``, re-initialize the parameter values of dihedrals from
        reference data hesssians after a MODIFY operation changes their
        periodicities. If ``False``, do not perform this reset.
    dihedral_periodicity_reset_max_n
        The upper bound for a periodicity produced by a dihedral periodicity
        reset.
    dihedral_periodicity_reset_alpha
        Alpha value for a dihedral periodicity reset.
    dihedral_periodicity_reset_min_k
        The lower bound for a stiffness produced by a dihedral periodicity
        reset.
    dihedral_periodicity_reset_max_k
        The upper bound for a stiffness produced by a dihedral periodicity
        reset.

    steps
        Steps in the strategy. Initialized as empty and then configured
        automatically by the ``build_steps()`` method.
    reference_list
        Configured automatically by the ``build_steps()`` method.
        The reference list to use for merge protect and target splitting
        configuration options, below. Because parameters can be added later, we
        only try to protect the original (reference) set.

    # Do not merge these
    # Configured in build_steps()
    merge_protect_list
        Configured automatically by the ``build_steps()`` method.
        Entries of ``reference_list`` that should not be included in MERGE
        operations.

    # Only operate on these
    target_list
        Configured automatically by the ``build_steps()`` method.
        Entries of ``reference_list`` that should be targeted during operations;
        other parameters are not operated on.

    filter_above
        Configured automatically by the ``build_steps()`` method. Remove
        candidates that have an estimated objective difference above this value
        If ``None``, this pruning is not performed. If ``0.0``, prune anything
        that is deemed useless.
    keep_below
        Configured automatically by the ``build_steps()`` method. Keep
        candidates that have an estimated objective difference below this value.
    """
