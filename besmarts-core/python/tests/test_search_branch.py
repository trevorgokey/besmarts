"""
besmarts.tests.test_search_branch
"""
...
primitives = codec_native.primitive_codecs_get()
bond_p = ["bond_order", "bond_ring"]
atom_p = [k for k in primitives if k not in bond_p]
# gcd = codec_native.graph_codec_native(primitives, ["element"], ["bond_order"])
gcd = codec_native.graph_codec_native(primitives, atom_p, bond_p)

buf_in = build_input()
A = codec_native.graph_codec_native_read(buf_in)[0]
print(gcd.smiles_encode(A))

torsions = graphs.graph_to_structure_dihedrals(A)[:2]

depth = 2
mapper.mapper_smarts_extend(configs.smarts_extender_config(depth, depth, True), torsions)

t = codec_native.graph_codec_native_read(build_torsion())

config = configs.mapper_config(1, False, "high")
t_union = mapper.union_list_parallel(torsions, config)
print("Structures are")
[print(gcd.smarts_encode(x)) for x in torsions]
print("Union is")
print(gcd.smarts_encode(t_union))

branches = 2

B = graphs.structure_branch(t_union, {k:k for k in t_union.select[:4]}, branches, depth)
B = list(B)
...
for b in B:
    print(len(b.nodes), graphs.structure_max_depth(b), sorted(b.nodes.keys()), gcd.smarts_encode(b))
