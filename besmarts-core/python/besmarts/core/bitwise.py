...
from besmarts.core import mappings

from besmarts import graphs
from besmarts import topology

def align_score(g: graphs.structure, h: graphs.structure, t):
    """
    ...
    return graphs.structure_bits(subgraph_intersection(g, h, t))

def align_score_parallel(indices):
    ref = align_score_ctx.ref
    to_check = align_score_ctx.to_check
    return tuple(tuple((i, align_score(ref, to_check[i][0], to_check[i][1]))) for i in indices)


def ordered_align_score(i, ref, o, t):
    return i, align_score(ref, o, t)
...
def subgraph_xor(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_xor)

def subgraph_xor_conditional_left(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_xor_cl)

def subgraph_xor_conditional_right(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_xor_cr)

def subgraph_subtract(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_subtract)

def subgraph_subtract_conditional_left(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_subtract_cl)

def subgraph_subtract_conditional_right(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_subtract_cr)

def subgraph_union(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_or)

def subgraph_intersection(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_and)

def subgraph_intersection_conditional_left(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_and_cl)

def subgraph_intersection_conditional_right(g: graphs.subgraph, h: graphs.subgraph, t: dict) -> graphs.subgraph:
    return subgraph_bitwise(g, h, t, chem.bechem_and_cr)

def subgraph_bitwise(g: graphs.subgraph, h: graphs.subgraph, t: dict, fn) -> graphs.subgraph:

    q = graphs.graph({}, {})
    adj = graphs.graph_connections(g)
    m = {}

    for i, a in enumerate(g.select, 1):
        b = t[a]
        q.nodes[i] = fn(g.nodes[a], h.nodes[b])
        m[a] = i

    for ai in adj:
        for aj in adj[ai]:
            e = graphs.edge((m[ai], m[aj]))
            if e not in q.edges:
                e_g = graphs.edge((ai, aj))
                e_h = graphs.edge((t[ai], t[aj]))
                q.edges[e] = fn(g.edges[e_g], h.edges[e_h])
    return graphs.graph_to_subgraph(q, tuple(range(1, len(g.select)+1)))

def subgraph_neg(g: graphs.subgraph) -> graphs.subgraph:
    g = graphs.subgraph_remove_unselected(g)
